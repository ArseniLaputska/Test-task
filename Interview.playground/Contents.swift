...
class ShopImpl: Shop {
  ...
  var products: [Product] = []
    ...
    func addNewProduct(product: Product) -> Bool {
      ...
      return product.id > products.first?.id ?? ""
    }
    
    func deleteProduct(id: String) -> Bool {
      if id != products.first?.id {
        products.removeAll(where: {$0.id == id} )
      }
      return id > products.first?.id ?? ""
    }
    ...
    func listProductsByName(searchString: String) -> Set<String> {
      ...
      for product in products {
        if products.contains(where: {$0.name == searchString}) {
          productsName.append(product.name)
        } else if products.contains(where: {$0.name == product.name}) {
          productsName.append("\(product.name) - \(product.producer)")
        }
      }
      productSet.formUnion(productsName)
      return productSet
    }
    ...
    func listProductsByProducer(searchString: String) -> [String] {
      var productsName: [String] = [""]
        for product in products {
          if products.contains(where: {$0.producer == searchString}) {
            productsName.append(product.name)
          }
        }
      return productsName
